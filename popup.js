...
function createSuggestionCard() {
    ...
    const logoUrl = chrome.runtime.getURL('LogK.png');

    suggestionCard.innerHTML = `
        <div class="kh-card-header">
            <div class="kh-header-logo-group">
                <img src="${logoUrl}" alt="KeyEZ Logo" class="kh-header-logo">
                <span class="kh-header-title"><span class="kh-title-key">Key</span><span class="kh-title-ez">EZ</span></span>
            </div>
...
function processResults(data, text) {
    const { tokens, errors, token_offsets } = data;

    // Build error list with positions
    currentErrors = [];

    tokens.forEach((token, index) => {
        // Handle both Array and Object (dict) formats for errors
        let error = null;
        if (Array.isArray(errors)) {
            error = errors.find(e => e.index === index);
        } else if (errors && typeof errors === 'object') {
            error = errors[index];
        }

        if (error && !ignoredWords.has(token)) {
            // Use the provided offset if available, otherwise calculate it
            const startPos = (token_offsets && token_offsets[index] !== undefined)
                ? token_offsets[index]
                : text.indexOf(token); // Fallback search (less reliable for repetitions)

            // Use the full error token text (which might be merged from multiple tokens)
            const errorWord = error.token || error.original || token;

            currentErrors.push({
                word: errorWord,
                start: startPos,
                end: startPos + errorWord.length,
                suggestions: error.suggestions ? error.suggestions.map(s => typeof s === 'string' ? s : s.word).slice(0, 5) : [],
                index: currentErrors.length,
                error_type: error.error_type || 'spelling'
            });
        }
    });
...
function showSuccessState() {
    ...
    cardBody.innerHTML = `
        <div class="kh-success-state">
            <div class="kh-success-icon-container">
                <svg class="kh-success-icon-svg" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                     <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </svg>
            </div>
            <h3 class="kh-success-title">Great Work!</h3>
...
function showAllErrorsPanel() {
    ...
    currentErrors.forEach((error, index) => {
        ...
        // Add contextual class for blue border
        if (error.error_type === 'contextual') {
            errorItem.classList.add('contextual');
        }
